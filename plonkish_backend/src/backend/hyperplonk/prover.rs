use crate::{
    ...
    util::{
        arithmetic::{div_ceil, product, steps_by, sum, BatchInvert, PrimeField},
        chain, end_timer,
        expression::{
...
pub(crate) fn permutation_z_polys<F: PrimeField, R: Rotatable + From<usize>>(
    ...
    let mut z = vec![vec![F::ZERO; 1 << num_vars]; num_chunks]; // 注意z多项式最好把初始值设置成0
...
pub(crate) fn cross_lookup_s_polys<F: PrimeField + Hash, R: Rotatable + From<usize>>(
    cross_system_polys: &[usize],
    polys: &[impl Borrow<MultilinearPolynomial<F>>],
    r: &F,
) -> Vec<MultilinearPolynomial<F>> {
    let polys = polys.iter().map(Borrow::borrow).collect_vec();
...
pub(crate) fn cross_lookup_z_polys<F: PrimeField, R: Rotatable + From<usize>>(
    cross_system_polys: &[usize],
    polys: &[impl Borrow<MultilinearPolynomial<F>>],
    s_polys: &[impl Borrow<MultilinearPolynomial<F>>],
    r: &F,
) -> Vec<MultilinearPolynomial<F>> {
    let polys = polys.iter().map(Borrow::borrow).collect_vec();
    let s_polys = s_polys.iter().map(Borrow::borrow).collect_vec();
    let num_vars = polys[0].num_vars();
    let usable_indices = R::from(num_vars).usable_indices();

    let cross_lookup_z_polys = cross_system_polys
        .iter()
        .enumerate()
        .map(|(idx, poly_idx)| {
            ...
            MultilinearPolynomial::<F>::new(zs)
        })
        .collect();
